...
Elab.Mapbox = (function (Elab) {
  ...
  function getLayerColors(range, colors, gradientType, cutoffs) {
    ...
    var colorScale = getColorScale(positions, colors, gradientType);
    var colorSteps = [];
  ...
  function getCssGradient(colors, gradientType) {
    ...
    colorScale = getColorScale(positions, colors, gradientType);
  ...
  function getColorScale(range, colors, gradientType) {
    // if (gradientType === "diverging") {
    //   // Simple three-color diverging using linear interpolation
    //   const dom = [range[0], range[0] / 2 + range[1] / 2, range[1]];
    //   console.log(dom, colors, d3.interpolateRgb);
    //   return d3
    //     .scaleLinear()
    //     .domain(dom) // [min, mid, max]
    //     .range(colors) // [lowColor, midColor, highColor]
  ...
  function createMap(el, config) {
    ...
    function handleLoad() {
      d3.json(geojsonUrl, function (err, json) {
        ...
        d3.csv(dataUrl, function (data) {
          ...
          colorScale = getColorScale(range, colors, gradientType);
    ...
    function renderLegend() {
      ...
      var titleContainer = rootEl.find(".legend__title");
      var range = getRange(currentProp);
      var linearGradient = getCssGradient(colors, gradientType);
