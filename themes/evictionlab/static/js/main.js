...
function setupContactForm() {
  ...
  form.attr(
    "action",
    formActionBase + $("input:radio[name=routing]:checked").data("email")
  );
...
function populateCovidRows(items) {
  ...
  items.forEach((item) => {
    if (String(item.linktosourcenewspressreleaseetc).length > 30) {
      item.linktruncated =
        item.linktosourcenewspressreleaseetc.trim().substring(0, 30) + "...";
    }
    var rowMarkup =
      '<tr class="">' +
      "<td>" +
      ...
      "</td>" +
      "<td>" +
      item.levelofgovernmentlocalstatenational +
      "</td>" +
      "<td>" +
      item.typeofaction +
      "</td>" +
      ...
      "</td>" +
      "<td>" +
      (!!item.linktosourcenewspressreleaseetc
        ? '<a href="' +
          item.linktosourcenewspressreleaseetc +
          '" target="_blank">' +
          item.linktruncated +
...
$(document).ready(function () {
  ...
  var setSplashHeight = function () {
    ...
    if (height > 1) el.css("height", height + "px");
  };
  if (el.length) {
      ...
      $("html,body").animate(
        {
          scrollTop:
            target.offset().top -
            (getWindowOffset() + getPageOffset(location.pathname)) +
            "px",
        },
        1000
...
function createMediaTable(
  ...
  var lmsReferralStr =
    totalLMSReferrals > 0 ? ", " + totalLMSReferrals + " LMS referrals" : "";
...
$(document).ready(function () {
  var SHEET_BASE =
    "https://spreadsheets.google.com/feeds/list/1fIHfkpG134ax5neHQGfipUrWbzPd-EAlVVlYqm4i9iE/";
